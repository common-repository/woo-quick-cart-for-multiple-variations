...
/**
 * This function update meta query to get only products which meta key for variation table is not exist.
 * @param $query
 * @param $query_vars
 *
 * @return mixed
 */
function wqcmv_handle_custom_query_var( $query, $query_vars )
{
    if ( !empty($query_vars['product_visiblity_option']) ) {
        $query['meta_query'][] = array(
            'key'     => 'product_visiblity_option',
            'compare' => esc_attr( $query_vars['product_visiblity_option'] ),
        );
    }
    return $query;
}
